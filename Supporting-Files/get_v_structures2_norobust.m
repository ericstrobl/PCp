function [pdag, cell_p, num_struc] = get_v_structures2_norobust(G, sep, cell_p, num_struc, k, cond_indep, varargin)
...
%               [~,p2t]=partialcorr(data(:,x),data(:,z),data(:,[z,cond]));
          ...
          cell_p2{z,y}=[cell_p2{z,y};   min([cell_p{x,y},p2])]; %*******
          cell_p2{x,y}=[cell_p2{x,y};   min([cell_p{z,y},p2])]; %*******
...
for t=1:length(idx),
    cell_p{idx(t)}=min([cell_p{idx(t)},sum(cell_p2{idx(t)})]);
end
